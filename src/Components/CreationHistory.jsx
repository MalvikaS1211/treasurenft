...
import { createHistory } from "../Helper/API_Functions";
import { useAccount } from "wagmi";
import moment from "moment";
export default function CreationHistory() {
  const { address } = useAccount();
  // const address = "0x52054a7F8f8e9F77C5B23A937943781b8F482b07";
  ...
  const handleTableData = async () => {
    const res = await createHistory(address, currentPage, itemPerpage);
                      ...
                      tabledata?.map((data, index) => (
                        <tr key={index}>
                          ...
                          <td>
                            ...
                            {data.creator.slice(-7)}
                          </td>
                          <td>{(data?.price / 1e18).toFixed(4)}</td>
                          <td>{data?.tokenId}</td>
